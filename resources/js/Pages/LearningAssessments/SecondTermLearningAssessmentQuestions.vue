<template>
 ...
 export default {
     name: "SecondTermLearningAssessmentQuestions",
     components: {
         LoadingSpinner
     },
     props: {
     ...
     methods: {
             ...
             axios.post(route('store.user-que-ans'), {
                 ...
                 .then((response) => {
                     if (response.data.status === "Success") {
                         window.location.href = "/second-term/completed-disc-assessment/" + this.childDetail.id;
                     }
                 })
                 .catch((error) => {
         ...
         fetchQuestions() {
             ...
             var getQuestionUrlEndPoint = this.assessmentApiUrl + `learn?type=${userType}` + `&part=2` ;   //part=2 for second term assessments questions;

<template>
    ...
    <div class="px-3 md:ml-[20rem]">
        <div class="rounded-l-lg pb-2">
            <slot />

            <!-- KIDS DISC REPORT DONE TYPES-->
            <div
                class="w-full flex mt-10 px-4"
                :class="{
                    'justify-between': this.active_index > 0 && this.active_index < 20,
                    '!justify-end': this.active_index === 0 || this.active_index === null,
                }"
            >
                <Link
                    v-if="this.active_index > 0"
                    class="unify_button py-2 px-5"
                    :href="route('secondTerm.showKidsDiscStyleReport', {
                        user: currentChildId,
                ...
                <Link
                    v-if="this.active_index < 20"
                    class="unify_button py-2 px-5"
                    :class="{'ml-auto': this.active_index === 0 || this.active_index === null}"
                    :href="route('secondTerm.showKidsDiscStyleReport', {
                        user: currentChildId,
                        type: this.kidsDiscReportSidebarMenus[this.active_index + 1].link,
                    })"
    ...
    methods: {
        ...
        getSideBarMenuName(type) {
            ...
            if (isDynamicData && type?.childNameNeeded) {
                ...
                const style = this.$attrs?.discStyle ;
                return text.replace('[Disc Style]', style);
            } else {
                return text;
            }
        },
        setActivePageAndIndex() {
            console.log('called')
            this.active_index = this.kidsDiscReportSidebarMenus.findIndex(
                (elem) => {
                    return (
                        elem.link ===
                        this.$page.url.split("/")[

...
<div style="border:none;width:100%;background-color:#dcb4e6;color:#333;font-family:Arial;font-size:14px;">
    ...
    <div style="padding:30px 20px 20px 20px;max-width:500px;margin:0px auto;background-color:#ffffff;">
        <div style="font-size:24px;font-family:Arial; text-align:center; font-weight:bold">
                You Session to {{$coach->first_name }} {{$coach->last_name}} Has Been  <span style="color:purple"> {{$proposal->status->name}} </span> !
        </div>
        {{-- <div style="margin: 50px 0px; text-align: center">
            @if($session->status == \App\Enums\StatusEnum::Accepted)
                <a href="{{route("sessions.pay" , $session->id)}}" style="text-decoration:none;;margin:auto auto ; ;text-align:center;color:white; background-color:#170048; padding:20px 40px; border-radius:45px;"> Pay For Session  </a>
            @elseif($session->status == \App\Enums\StatusEnum::Declined)
                <a href="{{route("users.show_coach" , $coach->id)}}" style="text-decoration:none;;margin:auto auto ; ;text-align:center;color:white; background-color:#170048; padding:20px 40px; border-radius:45px;"> Submit Another Proposal </a>
            @endif
        </div> --}}
